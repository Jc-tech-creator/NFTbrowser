import { Card, Image, List, Skeleton, Space } from "antd";
import ContractNfts from "./ContractNfts";
import ContractTrades from "./ContractTrades";

const fallbackSrc = `data:image/png;base64,`;

const NftCard = ({ nft }) => {
  console.log(JSON.parse(nft.metadata));
  const { image, name } = JSON.parse(nft.metadata);
  // https://ipfs.github.io/public-gateway-checker/
  const imageUrl = image?.replace(
    "ipfs://",
    "https://ipfs.io/ipfs/"
    // "https://dweb.link/ipfs/"
    // "https://cloudflare-ipfs.com/ipfs/"
  );

  return (
    // List is already defined in app.js, for each nft, I only define List.Item here
    <List.Item key={nft.token_id}>
      <Card
        title={name}
        extra={
          // extra 是右上角的区域，in this case, there are two buttons, and two components inside
          <Space>
            <ContractNfts tokenAddress={nft.token_address} />
            <ContractTrades tokenAddress={nft.token_address} />
          </Space>
        }
      >
        <Image
          src={imageUrl}
          width="100%"
          height={200}
          fallback={fallbackSrc} // it fail to give image, display a default image
          placeholder={<Skeleton active />} // this is make sure when loading there is loading style
        />
      </Card>
    </List.Item>
  );
};

export default NftCard;
